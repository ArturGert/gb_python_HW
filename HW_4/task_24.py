...
berries = list(map(int, input().split()))  # урожайность кустов

max_berries = 0  # максимальное количество ягод
for i in range(n):
    # находим сумму ягод на i-ом кусте и двух соседних с ним
    total = berries[i] + berries[(i+1)%n] + berries[(i-1)%n]
    # обновляем максимальное количество ягод
    max_berries = max(max_berries, total)
